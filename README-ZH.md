...
- **確認與誠實**（E2E-1113）：最後一個 CDN 邊緣不能排空／刪除；VNC `userdel` 失敗唔當成功；進程 TERM／KILL 要先確認（KILL 要打 `KILL`）；GeoIP 過舊用檔案時間。
- **更好讀** — 時區顯示主機時間對瀏覽器；系統變更／超級用戶／草稿／正常已翻譯；還原預覽檔數係真實 tar 條數。
- **Docker／驗證者** 刪除仍要打名稱；空容器日誌會說明行程已結束、沒有輸出。
