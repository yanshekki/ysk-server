...
import {
  ...
  normalizeLocale,
  runWithLocaleAsync,
  type StructuredResult,  tl} from '@yanshekki/shared';
import {
...
async function mainInner(
  ...
  if (command === 'tools') {
    ...
    if (sub === 'run') {
      ...
      const { executeToolCall } = await import('@yanshekki/core');
      const configPath = getOpt(args, '--config');
  ...
  if (command === 'email') {
    ...
    try {
      ...
      if (sub === 'bootstrap') {
        const { bootstrapEmailServer } = await import('@yanshekki/core');
      ...
      if (sub === 'queue') {
        const act = args.filter((a) => !a.startsWith('-')).slice(2)[0] ?? 'list';
        const { listMailQueue, flushMailQueue } = await import('@yanshekki/core');
      ...
      if (sub === 'relay') {
        ...
        const { applySmtpRelay, loadSmtpRelaySettings } = await import('@yanshekki/core');
    ...
    try {
      ...
      if (sub === 'shares') {
        ...
        if (act === 'bt-stats') {
          ...
          const { getFileShareById, collectBtShareStats } = await import('@yanshekki/core');
          ...
          const site = upsertCdnSite(ctx.db, {
            ...
            origin: originProject
              ...
              : originUrl
                ...
                : undefined,
            mode: getOpt(args, '--mode') as import('@yanshekki/shared').CdnSiteMode | undefined,
          });
          printJson({ ok: true, site });
          return 0;
        }
  ...
  if (command === 'agents') {
    ...
    if (sub === 'probe' || hasFlag(args, '--probe')) {
      ...
      const { createAppContext, closeAppContext } = await import('./app-context.js');
      const { probeAllAgentRuntimes } = await import('@yanshekki/core');
  ...
  if (command === 'agent') {
    ...
    const { runOutboundAgent } = await import('@yanshekki/core');
    ...
    await runOutboundAgent({
      ...
      onCommand: async (cmd) => {
        ...
        if (payload?.op === 'clusterSync' && payload.cluster) {
          const { importDbClusterSync } = await import('@yanshekki/core');
          const dataDir =
            getOpt(args, '--data-dir') ??
            process.env.YSK_DATA_DIR ??
            join(process.cwd(), '.ysk');
          ...
          try {
            const r = importDbClusterSync({
              ...
              cluster: payload.cluster as import('@yanshekki/core').DbCluster });
            return {
              ok: r.ok,
              exitCode: r.ok ? 0 : 1,
              op: 'clusterSync',
  ...
  if (command === 'templates') {
    const { listAppTemplates } = await import('@yanshekki/core');
  ...
  if (
    ...
    const { createAppContext, closeAppContext } = await import('./app-context.js');
    const { applyListQuery } = await import('@yanshekki/core');
    const { parseListQuery } = await import('@yanshekki/shared');
    const ctx = createAppContext({
      version: VERSION,
      config,
      configPath,
    ...
    try {
      ...
      if (command === 'security') {
        ...
        if (sub === 'api-keys' || sub === 'api-key' || sub === 'apikeys') {
          const action = args.filter((a) => !a.startsWith('-')).slice(2)[0] ?? 'list';
          const { listApiKeys, createApiKey, deleteApiKey } = await import('@yanshekki/core');
          if (action === 'list') {
  ...
  if (command === 'projects') {
    ...
    try {
      ...
      if (sub === 'isolation') {
        const action = args.filter((a) => !a.startsWith('-')).slice(2)[0] ?? 'list';
        const { listIsolationReport, backfillProjectOwners } = await import('@yanshekki/core');
  ...
  if (command === 'hosting') {
    ...
    const {
      ...
      applyEmailStack,
      applyFirewall } = await import('@yanshekki/core');
    const ctx = createAppContext({
      version: VERSION,
      config,
      configPath,
    ...
    try {
      ...
      if (sub === 'ftps-apply') {
        const { applyFtps } = await import('@yanshekki/core');
        const domain = getOpt(args, '--domain');
        if (!domain) {
      ...
      if (sub === 'runtimes' || sub === 'runtimes-probe') {
        const { probeRuntimes, listSupportedRuntimes } = await import('@yanshekki/core');
        try {
          printJson({
            ok: true,
      ...
      if (sub === 'runtime-switch') {
        ...
        const { switchRuntimeDefault, defaultRuntimeVersion } = await import('@yanshekki/core');
        const kindRaw = getOpt(args, '--kind') ?? 'node';
        const allowed = [
          'node',
          'php',
      ...
      if (sub === 'dovecot-passdb') {
        const { writeDovecotPassdb, writeAllDovecotPassdbs } = await import('@yanshekki/core');
        const domain = getOpt(args, '--domain');
      ...
      if (sub === 'webmail-apply') {
        const { applyWebmail } = await import('@yanshekki/core');
        const domain = getOpt(args, '--domain');
        if (!domain) {
      ...
      if (sub === 'public-files') {
        const { applyPublicFileServer } = await import('@yanshekki/core');
        const domain = getOpt(args, '--domain');
        if (!domain) {
  ...
  if (command === 'db-cluster') {
    ...
    const {
      ...
      importDbClusterSync } = await import('@yanshekki/core');
    const ctx = openCliContext(args);
    try {
      if (sub === 'list' || sub === 'ls') {
      ...
      if (sub === 'import-sync') {
        ...
        const data = JSON.parse(raw) as { cluster?: import('@yanshekki/core').DbCluster };
        if (!data.cluster) {
  ...
  if (command === 'ssh-key' || command === 'ssh-keys') {
    ...
    const {
      ...
      deleteSshIdentity,
      installSshIdentity,
      uninstallSshIdentity } = await import('@yanshekki/core');
    const ctx = openCliContext(args);
    try {
      if (sub === 'list' || sub === 'ls') {
  ...
  if (command === 'ssh-2fa' || command === 'ssh2fa') {
    ...
    const {
      ...
      buildSshdTotpHints,
      probeSsh2faHost,
      revealSsh2faSecret } = await import('@yanshekki/core');
    const ctx = openCliContext(args);
    try {
      if (sub === 'list' || sub === 'ls') {
  ...
  if (command === 'nginx') {
    ...
    const {
      ...
      syncNginxConfigs } = await import('@yanshekki/core');
    const ctx = openCliContext(args);
    try {
      ...
      if (sub === 'test' || sub === 'check') {
        {
          const { binPresent } = await import('@yanshekki/core');
  ...
  if (command === 'ssl') {
    ...
    try {
      ...
      if (sub === 'bootstrap' || sub === 'bootstrap-tls') {
        const { ensureBootstrapPanelTls } = await import('@yanshekki/core');
  ...
  if (command === 'host') {
    ...
    const { collectHostOverview, collectMetrics } = await import('@yanshekki/core');
    const ctx = openCliContext(args);
    try {
  ...
  if (command === 'logs') {
    ...
    const {
      ...
      getLogOverview,
      loadLogSettings,
      listJournalUnits } = await import('@yanshekki/core');
    const ctx = openCliContext(args);
    try {
  ...
  if (command === 'services') {
    ...
    const { getServiceMatrix, lifecycleServiceUnit } = await import('@yanshekki/core');
    const ctx = openCliContext(args);
    try {
  ...
  if (command === 'defense' || command === 'protection') {
    ...
    const {
      ...
      loadAutoBanPolicy,
      updateAutoBanPolicy,
      probeFirewallDeep } = await import('@yanshekki/core');
    const ctx = openCliContext(args);
    try {
      if (sub === 'status') {
      ...
      if (sub === 'bans' || sub === 'list-bans') {
        const { listDefenseBans, applyListQuery } = await import('@yanshekki/core');
        const { parseListQuery } = await import('@yanshekki/shared');
      ...
      if (sub === 'suspects') {
        const { listSuspectIps, applyListQuery } = await import('@yanshekki/core');
        const { parseListQuery } = await import('@yanshekki/shared');
      ...
      if (sub === 'presets') {
        const { listDefensePresets, getDefenseStatus } = await import('@yanshekki/core');
        const status = await getDefenseStatus({
          host: ctx.host,
          db: ctx.db,
          dataDir: ctx.dataDir,
  ...
  if (command === 'migrate') {
    ...
    const {
      ...
      runLocalMigratePost,
      loadMigrateJob,
      listMigrateJobs } = await import('@yanshekki/core');
    const ctx = createAppContext({
      version: VERSION,
      config,
      configPath,
  ...
  if (command === 'serve') {
    ...
    try {
      const { restoreBtSharesOnBoot } = await import('@yanshekki/core');
  ...
  if (command === 'readiness' || command === 'doctor') {
    const { assessProductionReadiness, storeStatus } = await import('@yanshekki/core');
    const dataDir = getOpt(args, '--data-dir') ?? join(process.cwd(), '.ysk');
    const ctx = createAppContext({
      version: VERSION,
      dataDir,
