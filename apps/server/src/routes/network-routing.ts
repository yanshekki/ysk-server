...
export async function handleNetworkRoutingRoutes(
  ...
  if (
    ...
    const { networkAddRoute, networkDelRoute } = await import('@yanshekki/core');
    const body = {
      host: ctx.host,
  ...
  if (method === 'PUT' && url.pathname === '/api/v1/network/dns') {
    ...
    const { networkSetDns } = await import('@yanshekki/core');
  ...
  if (method === 'POST' && url.pathname === '/api/v1/network/dns/test') {
    ...
    const { networkTestDns } = await import('@yanshekki/core');
