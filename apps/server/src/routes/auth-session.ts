/**
 * Auth session — login/logout/me/sessions/password/locale/api-keys.
...
export async function handleAuthSessionRoutes(
  ...
  method: string,
      ...
      if (method === 'GET' && url.pathname === '/api/v1/auth/api-keys') {
        const user = ctx.auth.authenticate(getBearer(req));
        const { listApiKeys } = await import('@yanshekki/core');
      ...
      if (method === 'POST' && url.pathname === '/api/v1/auth/api-keys') {
        ...
        const { createApiKey } = await import('@yanshekki/core');
      ...
      if (method === 'DELETE' && url.pathname.match(/^\/api\/v1\/auth\/api-keys\/[^/]+$/)) {
        const user = ctx.auth.authenticate(getBearer(req));
        const id = url.pathname.split('/')[5] ?? '';
        const { deleteApiKey } = await import('@yanshekki/core');
