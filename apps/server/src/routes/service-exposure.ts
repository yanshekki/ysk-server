...
export async function handleServiceExposureRoutes(
  ...
  if (method === 'GET' && url.pathname === BASE) {
    ...
    const { listDesired, getServiceExposureStatus } = await import('@ysk/core');
    const { YSK_SERVICE_PORTS, defaultPortsForService } = await import('@ysk/shared');
  ...
  if (method === 'GET' && oneMatch) {
    ...
    const { getServiceExposureStatus } = await import('@ysk/core');
  ...
  if (method === 'PUT' && oneMatch) {
    ...
    const { putServiceExposure } = await import('@ysk/core');
