...
export async function handleAuthMfaRoutes(
  ...
  method: string,
      ...
      if (method === 'GET' && url.pathname === '/api/v1/auth/webauthn/credentials') {
        const user = ctx.auth.authenticate(getBearer(req));
        const { listWebAuthnCredentials } = await import('@ysk-server/core');
        sendJson(res, 200, {
          ok: true,
      ...
      if (method === 'POST' && url.pathname === '/api/v1/auth/webauthn/register/begin') {
        ...
        const { beginWebAuthnRegistration } = await import('@ysk-server/core');
      ...
      if (method === 'POST' && url.pathname === '/api/v1/auth/webauthn/register/finish') {
        ...
        const { finishWebAuthnRegistration } = await import('@ysk-server/core');
      ...
      if (method === 'POST' && url.pathname === '/api/v1/auth/webauthn/authenticate/begin') {
        const user = ctx.auth.authenticate(getBearer(req));
        const { beginWebAuthnAuthentication } = await import('@ysk-server/core');
      ...
      if (method === 'POST' && url.pathname === '/api/v1/auth/webauthn/authenticate/finish') {
        ...
        const { finishWebAuthnAuthentication, markTotpStepUp } = await import('@ysk-server/core');
      ...
      if (method === 'GET' && url.pathname === '/api/v1/auth/devices') {
        const user = ctx.auth.authenticate(getBearer(req));
        const { listRememberDevices } = await import('@ysk-server/core');
        sendJson(res, 200, {
          ok: true,
      ...
      if (method === 'DELETE' && url.pathname === '/api/v1/auth/devices') {
        const user = ctx.auth.authenticate(getBearer(req));
        const { revokeAllRememberDevices } = await import('@ysk-server/core');
      ...
      if (method === 'POST' && url.pathname === '/api/v1/auth/totp/backup') {
        ...
        const { exportTotpBackup } = await import('@ysk-server/core');
      ...
      if (method === 'DELETE' && url.pathname.match(/^\/api\/v1\/auth\/webauthn\/credentials\/[^/]+$/)) {
        ...
        const { deleteWebAuthnCredential } = await import('@ysk-server/core');
      ...
      if (method === 'DELETE' && url.pathname.match(/^\/api\/v1\/auth\/devices\/[^/]+$/)) {
        ...
        const { revokeRememberDevice } = await import('@ysk-server/core');
