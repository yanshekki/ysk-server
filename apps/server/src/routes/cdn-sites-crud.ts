...
export async function handleCdnSitesCrudRoutes(
  ...
  method: string,
      ...
      if (method === 'GET' && url.pathname === '/api/v1/cdn/sites') {
        ...
        const { listCdnSites } = await import('@yanshekki/core');
        type Site = {
          id?: string;
          name?: string;
          domains?: string[];
      ...
      if (method === 'POST' && url.pathname === '/api/v1/cdn/sites') {
        ...
        const { upsertCdnSite } = await import('@yanshekki/core');
      ...
      if (method === 'POST' && url.pathname === '/api/v1/cdn/from-project') {
        ...
        const { enableCdnFromProject } = await import('@yanshekki/core');
        const r = enableCdnFromProject({
          db: ctx.db,
          project: proj,
          edgeNodeIds: data.edgeNodeIds,
          originShieldNodeId: data.originShieldNodeId,
          strategy: data.strategy as import('@yanshekki/shared').CdnDnsStrategy | undefined,
          mode: data.mode as import('@yanshekki/shared').CdnSiteMode | undefined,
          geoMap: data.geoMap,
      ...
      if (method === 'POST' && url.pathname === '/api/v1/cdn/health-loop') {
        ...
        const { runAllCdnSitesHealthLoop } = await import('@yanshekki/core');
      ...
      if (method === 'GET' && url.pathname === '/api/v1/cdn/dashboard') {
        ctx.auth.authenticate(getBearer(req));
        const { collectCdnDashboard } = await import('@yanshekki/core');
      ...
      if (method === 'GET' && url.pathname.match(/^\/api\/v1\/cdn\/sites\/[^/]+$/)) {
        ...
        const { getCdnSite, readCdnSiteRenderedConf } = await import('@yanshekki/core');
      ...
      if (method === 'DELETE' && url.pathname.match(/^\/api\/v1\/cdn\/sites\/[^/]+$/)) {
        const user = ctx.auth.authenticate(getBearer(req));
        const id = url.pathname.split('/')[5];
        const { deleteCdnSite } = await import('@yanshekki/core');
