...
export async function handleCdnSitesSslRoutes(
  ...
  if (
    ...
    const { distributeCdnSiteSsl } = await import('ysk-server-core');
  ...
  if (
    ...
    const { issueCdnSiteLetsEncrypt } = await import('ysk-server-core');
  ...
  if (
    ...
    const user = ctx.auth.authenticate(getBearer(req));
    const id = url.pathname.split('/')[5];
    const { prepareCdnSiteAcme } = await import('ysk-server-core');
