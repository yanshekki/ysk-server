...
| Git control | `ysk-server projects git status\|log\|fetch\|checkout\|reset --id UUID` | read / write-host | Status/log are read-only. Dirty trees block pull. `.env` is restored from the control plane after sync. |
...
1. Project **App** tab → **Enable hook** → copy the full URL and the one-time secret.
