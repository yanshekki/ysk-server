...
2. 我在其中一台當 **control plane**，把其他機登錄為 **edge / origin / dns** 節點。
...
| 關一 edge | 健康失敗後 dig 只剩存活 IP（minHealthyEdges 保護） |
| purge | 兩邊 edge 快取皆清 |
...
| **Origin-pull** | Edge miss 時回源拉內容並快取 | MVP |
...
- `ttlHealthy`：健康時（建議 30–120s，CDN 宜短）
- `ttlUnhealthy`：摘除過渡（可更短）
- **minHealthyEdges**：若健康數 < 門檻，**不摘除**（防全滅），改標 partial + alert
...
| 功能 | 說明 |
|------|------|
| Site CRUD | name、domains[]、mode、origin |
| Origin | `project`（本機 projectId）或 `url`（外部 upstream） |
| Edge 選擇 | edgeNodeIds[] |
| DNS 綁定 | zoneId + strategy + TTL 參數 |
| Cache 政策 | enabled、zoneSize、maxAge、bypassCookies、bypassAuth |
...
- **CdnNodeDto** — 節點
- **CdnSiteDto** — 站點政策 + apply_status / edge_status
