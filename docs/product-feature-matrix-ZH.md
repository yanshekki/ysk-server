...
**狀態:** 產品契約（準則來源）  
**重計：** 2026-08-22 — 對齊 **1.1.25**（主機 crontab 就地修改；驗證者質押後續步驟；CLI 對齊）。只連官方網站，不接錢包。shared／core／產品同一版本。標記對齊已交付程式（面板 + CLI + API），不是行銷。    
...
| K7 | 主機 crontab 行修改（就地、按用戶） | 弱 | 弱 | ✓ | Better |
...
| L1 nodes (Beta) | Validators 頁 + CLI；Docker Compose；不保管金鑰；實例頁展示後續步驟與公開身份（NEAR／Cosmos／AVAX） |
