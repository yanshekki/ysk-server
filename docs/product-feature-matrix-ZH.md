...
**狀態:** 產品契約（準則來源）  
**重計：** 2026-08-20 — 對齊 **1.1.20**（Cardano 掛 opcert 當 producer、Lighthouse v8／Nimbus／Agave 旗標、Avalanche ifconfigMe 加 TZ、精靈 RAM 警告、LogViewer 關鍵字欄）。只連官方網站，不接錢包。shared／core／產品同一版本。標記對齊已交付程式（面板 + CLI + API），不是行銷。    
