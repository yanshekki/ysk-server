...
> {
  ...
  const hasLe =
    existsSync(`${certBase}/fullchain.pem`) && existsSync(`${certBase}/privkey.pem`);
  const mainCf = [
    ...
    ...(hasLe
      ...
      : ['# smtpd_tls_* omitted — LE files missing; do not point at absent paths']),
  ...
  const dovecot = [
    ...
    hasLe ? 'ssl = required' : 'ssl = no',
    ...(hasLe
      ? [`ssl_cert = <${certBase}/fullchain.pem`, `ssl_key = <${certBase}/privkey.pem`]
      : ['# ssl_cert omitted — LE files missing; Dovecot must not start with a dead path']),
