...
export async function backupProject(input: {
  ...
  const excludeArgs = [...defaultExcludes, ...(input.excludes ?? [])].flatMap((e) => [
    '--exclude',
    e,
  ]);
...
export class CronJobService {
  ...
  async installCrontab(actor: string): Promise<{
      ...
      if (cur.exitCode === 0 && cur.stdout) {
        preserved = cur.stdout
          ...
          .filter((l) => {
            ...
            if (t.startsWith('# YSK Server crontab merge')) return false;
    ...
    this.db.snapshot.settings.last_cron_install = JSON.stringify({ ok, at, actor });
    for (const j of this.db.snapshot.cron_jobs) {
      j.last_install = { ok, at, actor };
    }
    this.db.persist();
    return {
      ok,
  ...
  async probeInstallStatus(): Promise<{
    ...
    let last = all.map((j) => j.last_install).find((x) => x && typeof x === 'object') as
      | { ok?: boolean; at?: string }
      | undefined;
    if (!last) {
      try {
        const raw = this.db.snapshot.settings?.last_cron_install;
        if (raw) last = JSON.parse(raw) as { ok?: boolean; at?: string };
      } catch {
        /* ignore */
      }
    }
