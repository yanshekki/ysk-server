...
export async function planOrInstallRuntime(input: {
  ...
  if (input.kind === 'node') {
    ...
    // Setup steps match https://packages.sury.org/php/README.txt
    script = [
      '#!/usr/bin/env bash',
      `# YSK Server — install PHP ${plan.version} + selected extensions (sury.org)`,
      'set -euo pipefail',
      'export DEBIAN_FRONTEND=noninteractive',
      'export LC_ALL=C',
      '',
      '# Purge archived ondrej Launchpad PPA (fails apt-get update during migration)',
      ...
      '    [ -f "$f" ] || continue',
      '    if grep -qiE "ppa\\.launchpad\\.(net|content\\.com)/ondrej/php|launchpadcontent\\.com/ondrej/php" "$f" 2>/dev/null; then',
      '      rm -f "$f"',
      '    fi',
      '  done',
      'fi',
      '',
      'apt-get install -y lsb-release ca-certificates curl 2>/dev/null || true',
      '',
      'setup_sury_php() {',
      '  # Official: debsuryorg-archive-keyring + /etc/apt/sources.list.d/php.list',
      ...
      '  # Drop alternate list if we previously wrote one',
      ...
      '  echo "sury.org setup failed; trying distro packages only" >&2',
    ...
    notes.push('source=packages.sury.org/php (not Launchpad ondrej PPA)');
